import { Client as ChainSlicerClient, type Game } from './bindings';
...
/**
 * Service for interacting with the ChainSlicer game contract
 */
export class ChainSlicerService {
  private baseClient: ChainSlicerClient;
  private contractId: string;

  constructor(contractId: string) {
    this.contractId = contractId;
    // Base client for read-only operations
    this.baseClient = new ChainSlicerClient({
  ...
  private createSigningClient(
    ...
    return new ChainSlicerClient(options);
  }
  ...
  async prepareStartGame(
    ...
    const buildClient = new ChainSlicerClient({
      contractId: this.contractId,
      networkPassphrase: NETWORK_PASSPHRASE,
      rpcUrl: RPC_URL,
    ...
    const signedAuthEntry = await authorizeEntry(
      ...
      async (preimage) => {
        ...
        console.log('[prepareStartGame] preimage XDR type:', preimage.switch().name);
        const preimageBase64 = preimage.toXDR('base64');
        console.log('[prepareStartGame] preimage base64 length:', preimageBase64.length);
        ...
        const signResult = await player1Signer.signAuthEntry(
          preimageBase64,  // Preimage as base64 XDR
          {
            networkPassphrase: NETWORK_PASSPHRASE,
            address: player1,
          }
        ...
        console.log('[prepareStartGame] signedAuthEntry string length:', signResult.signedAuthEntry.length);
        console.log('[prepareStartGame] signedAuthEntry value:', signResult.signedAuthEntry.substring(0, 120));

        // Return signature as Buffer (authorizeEntry expects a Buffer)
        const sigBuffer = Buffer.from(signResult.signedAuthEntry, 'base64');
        console.log('[prepareStartGame] decoded signature byte length:', sigBuffer.length);
        console.log('[prepareStartGame] decoded signature hex:', sigBuffer.toString('hex').substring(0, 160));

        // authorizeEntry expects EXACTLY 64 bytes (raw ed25519 signature)
        // If we get something else, log the mismatch clearly
        if (sigBuffer.length !== 64) {
          console.error(`[prepareStartGame] ❌ SIGNATURE LENGTH MISMATCH: expected 64 bytes, got ${sigBuffer.length} bytes`);
          console.error('[prepareStartGame] This is likely the root cause of "signature of length 64 expected, got N"');
          console.error('[prepareStartGame] Raw signedAuthEntry from wallet:', signResult.signedAuthEntry);
        } else {
          console.log('[prepareStartGame] ✅ Signature is 64 bytes as expected');
        }
  ...
  /**
   * Submit ZK proof to the contract via the `prove` method.
   */
  async submitProof(sessionId: number, playerAddress: string, proofHex: string | null, attestationHex: string | null,
    signer: Pick<contract.ClientOptions, 'signTransaction' | 'signAuthEntry'>, authTtlMinutes?: number) {
    const client = this.createSigningClient(playerAddress, signer);
    const proof = proofHex ? Buffer.from(proofHex, 'hex') : Buffer.alloc(0);
    const attestation = attestationHex ? Buffer.from(attestationHex, 'hex') : Buffer.alloc(0);
    const tx = await client.prove({
      session_id: sessionId,
    ...
    try {
      ...
      if (err instanceof Error && err.message.includes('Transaction failed')) {
        throw new Error('Transaction failed - check if the game session is still active');
      }
      throw err;
    }
  }
...
// Example: const chainSlicerService = new ChainSlicerService(contractId);