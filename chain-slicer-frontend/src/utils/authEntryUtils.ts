  ...
  if (player2AuthEntry && player2Index !== -1) {
    ...
    const player2SignedAuthEntry = await authorizeEntry(
      ...
      async (preimage) => {
        ...
        console.log('[injectSignedAuthEntry] Player 2 signedAuthEntry string length:', signResult.signedAuthEntry.length);
        const sigBuffer = Buffer.from(signResult.signedAuthEntry, 'base64');
        console.log('[injectSignedAuthEntry] Player 2 decoded signature byte length:', sigBuffer.length);
        if (sigBuffer.length !== 64) {
          console.error(`[injectSignedAuthEntry] ❌ P2 SIGNATURE LENGTH MISMATCH: expected 64 bytes, got ${sigBuffer.length} bytes`);
        }
